...
function App() {
  ...
  async function handleSubmit(e){
    ...
    const response = await fetch("https://45.142.236.122:3080/", {
      method: "POST",
      headers: {
...
const ChatMessage = ({ message }) => {
  useEffect(() => {
    // Vérifie si l'utilisateur est "me"
    if (message.user === "me") {
  ...
  return (
    ...
    {message.user === "me" && (
          <div className={`avatar ${message.user === "gpt" && 'chatgpt'}`}>
            <img id={`avatar-${message.user}`} className='avatar' alt='' />
          </div>
        )}
      <div className={`avatar ${message.user === "gpt" && "chatgpt"}`}>
